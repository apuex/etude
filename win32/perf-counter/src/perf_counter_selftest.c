/*  =========================================================================
    perf_counter_selftest.c - run selftests
...
// Now built only with --enable-drafts, so even stable builds are hidden behind the flag
    { "perf_counter_it", NULL, true, false, "perf_counter_it_test" },
...
{
    test_item_t *item;
    printf ("Running perf-counter selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef PERF_COUNTER_BUILD_DRAFT_API // selftest is still in draft
        else
            perf_counter_private_selftest (verbose, item->subtest);
#endif // PERF_COUNTER_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("perf_counter_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running perf-counter test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef PERF_COUNTER_BUILD_DRAFT_API // selftest is still in draft
        else
            perf_counter_private_selftest (verbose, test->subtest);
#endif // PERF_COUNTER_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
